...
class ParquetData:
  ...
  def read_all(self, path, df_list, landmark_id, max_length = 140, SortSave=False):
    """
    ...
    for participant_folder in os.listdir(path + "/train_landmark_files/"):
      ...
      for file_name in os.listdir(path_folder):
        ...
        if(df_row.length_frames.values[0] > max_length):
          ...
          if SortSave == True:
            self.save_tensor(path_tensor, sign_name, self.data[sign_name][participant_folder].tensor, count)
...
data_load.read_all(path, df_train, selected_landmark_indices, 140, True)
